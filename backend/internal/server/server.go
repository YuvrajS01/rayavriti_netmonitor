...
func (s *Server) Start() error {
	...
	resourceH := handlers.NewResourceHandler(s.db)
	...
	reportGenH := handlers.NewReportGenHandler(s.db, s.cfg.Integrations.ReportOutputDir)
	...
	r.Get("/status", resourceH.PublicStatusHTML)
	r.Get("/api/v1/public/status", statusPageH.PublicStatusJSON)
	r.Get("/api/v1/public/incidents", resourceH.List("status_page_incidents"))
	...
	r.Group(func(r chi.Router) {
		...
		r.With(rbac.RequirePermission(models.PermSystemMonitoring)).Get("/api/v1/phase2/summary", resourceH.Summary)
		if monitoringH != nil {
		...
		r.With(rbac.RequirePermission(models.PermAlertsRead)).Get("/api/v1/alerts/suppressed", resourceH.List("suppressed_alerts"))
		...
		r.With(rbac.RequirePermission(models.PermReportsRead)).Get("/api/v1/reports/sla", resourceH.List("sla_definitions"))
		r.With(rbac.RequirePermission(models.PermReportsRead)).Get("/api/v1/reports/mttr", resourceH.List("incidents"))
		r.With(rbac.RequirePermission(models.PermReportsRead)).Get("/api/v1/reports/availability", report.Devices)
		r.With(rbac.RequirePermission(models.PermReportsRead)).Get("/api/v1/reports/isp", resourceH.List("isp_metrics"))
		r.With(rbac.RequirePermission(models.PermReportsRead)).Get("/api/v1/reports/top-offenders", report.Devices)
		r.With(rbac.RequirePermission(models.PermReportsWrite)).Get("/api/v1/reports/scheduled", resourceH.List("scheduled_reports"))
		r.With(rbac.RequirePermission(models.PermReportsWrite)).Post("/api/v1/reports/scheduled", resourceH.Create("scheduled_reports"))
		r.With(rbac.RequirePermission(models.PermReportsWrite)).Put("/api/v1/reports/scheduled/{id}", resourceH.Update("scheduled_reports"))
		r.With(rbac.RequirePermission(models.PermReportsWrite)).Delete("/api/v1/reports/scheduled/{id}", resourceH.Delete("scheduled_reports"))
		...
		r.With(rbac.RequirePermission(models.PermSettingsWrite)).Get("/api/v1/subnets", resourceH.List("subnets"))
		r.With(rbac.RequirePermission(models.PermSettingsWrite)).Post("/api/v1/subnets", resourceH.Create("subnets"))
		r.With(rbac.RequirePermission(models.PermSettingsWrite)).Get("/api/v1/subnets/{id}", resourceH.Get("subnets"))
		r.With(rbac.RequirePermission(models.PermSettingsWrite)).Put("/api/v1/subnets/{id}", resourceH.Update("subnets"))
		r.With(rbac.RequirePermission(models.PermSettingsWrite)).Delete("/api/v1/subnets/{id}", resourceH.Delete("subnets"))

		// --- Import (reports.read / import.execute) ---
		r.With(rbac.RequirePermission(models.PermReportsRead)).Get("/api/v1/import/history", resourceH.List("discovery_jobs"))
		...
		r.With(rbac.RequirePermission(models.PermStatusPageManage)).Get("/api/v1/status-page/services", resourceH.List("status_page_services"))
		r.With(rbac.RequirePermission(models.PermStatusPageManage)).Get("/api/v1/status-page/services/{id}", resourceH.Get("status_page_services"))
		r.With(rbac.RequirePermission(models.PermStatusPageManage)).Post("/api/v1/status-page/services", resourceH.Create("status_page_services"))
		r.With(rbac.RequirePermission(models.PermStatusPageManage)).Put("/api/v1/status-page/services/{id}", resourceH.Update("status_page_services"))
		r.With(rbac.RequirePermission(models.PermStatusPageManage)).Delete("/api/v1/status-page/services/{id}", resourceH.Delete("status_page_services"))
		...
		r.With(rbac.RequirePermission(models.PermStatusPageManage)).Post("/api/v1/status-page/incidents", resourceH.Create("status_page_incidents"))
		r.With(rbac.RequirePermission(models.PermStatusPageManage)).Put("/api/v1/status-page/incidents/{id}", resourceH.Update("status_page_incidents"))
		r.With(rbac.RequirePermission(models.PermStatusPageManage)).Post("/api/v1/status-page/incidents/{id}/update", resourceH.Create("status_page_incident_updates"))
		...
		r.With(rbac.RequirePermission(models.PermMaintenanceWrite)).Get("/api/v1/maintenance", resourceH.List("maintenance_windows"))
		r.With(rbac.RequirePermission(models.PermMaintenanceWrite)).Get("/api/v1/maintenance/active", resourceH.List("maintenance_windows"))
		r.With(rbac.RequirePermission(models.PermMaintenanceWrite)).Get("/api/v1/maintenance/calendar", resourceH.List("maintenance_windows"))
		r.With(rbac.RequirePermission(models.PermMaintenanceWrite)).Get("/api/v1/maintenance/{id}", resourceH.Get("maintenance_windows"))
		r.With(rbac.RequirePermission(models.PermMaintenanceWrite)).Post("/api/v1/maintenance", resourceH.Create("maintenance_windows"))
		r.With(rbac.RequirePermission(models.PermMaintenanceWrite)).Put("/api/v1/maintenance/{id}", resourceH.Update("maintenance_windows"))
		r.With(rbac.RequirePermission(models.PermMaintenanceWrite)).Delete("/api/v1/maintenance/{id}", resourceH.Delete("maintenance_windows"))
		r.With(rbac.RequirePermission(models.PermMaintenanceWrite)).Post("/api/v1/maintenance/{id}/toggle", resourceH.Update("maintenance_windows"))

		// --- Contacts & Escalation (contacts.write) ---
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Get("/api/v1/contacts", resourceH.List("contacts"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Get("/api/v1/contacts/{id}", resourceH.Get("contacts"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Post("/api/v1/contacts", resourceH.Create("contacts"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Put("/api/v1/contacts/{id}", resourceH.Update("contacts"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Delete("/api/v1/contacts/{id}", resourceH.Delete("contacts"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Get("/api/v1/contacts/{id}/devices", resourceH.List("device_contacts"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Post("/api/v1/devices/{id}/contacts", resourceH.Create("device_contacts"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Delete("/api/v1/devices/{id}/contacts/{cid}", resourceH.Delete("device_contacts"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Post("/api/v1/locations/{id}/contacts", resourceH.Create("device_contacts"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Get("/api/v1/escalation-policies", resourceH.List("escalation_policies"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Get("/api/v1/escalation-policies/{id}/steps", resourceH.List("escalation_steps"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Post("/api/v1/escalation-policies", resourceH.Create("escalation_policies"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Put("/api/v1/escalation-policies/{id}", resourceH.Update("escalation_policies"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Delete("/api/v1/escalation-policies/{id}", resourceH.Delete("escalation_policies"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Post("/api/v1/escalation-policies/{id}/steps", resourceH.Create("escalation_steps"))
		...
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Get("/api/v1/oncall", resourceH.List("oncall_schedules"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Get("/api/v1/oncall/schedule", resourceH.List("oncall_schedules"))
		r.With(rbac.RequirePermission(models.PermContactsWrite)).Put("/api/v1/oncall/{id}/override", resourceH.Update("oncall_schedules"))

		// --- Incidents (incidents.write) ---
		r.With(rbac.RequirePermission(models.PermIncidentsWrite)).Get("/api/v1/incidents", resourceH.List("incidents"))
		...
		r.With(rbac.RequirePermission(models.PermIncidentsWrite)).Get("/api/v1/incidents/{id}", resourceH.Get("incidents"))
		r.With(rbac.RequirePermission(models.PermIncidentsWrite)).Post("/api/v1/incidents", incidentH.CreateIncident)
		r.With(rbac.RequirePermission(models.PermIncidentsWrite)).Put("/api/v1/incidents/{id}", resourceH.Update("incidents"))
		...
		r.With(rbac.RequirePermission(models.PermUsersManage)).Get("/api/v1/users", resourceH.List("users"))
		r.With(rbac.RequirePermission(models.PermUsersManage)).Get("/api/v1/users/{id}", resourceH.Get("users"))
		r.With(rbac.RequirePermission(models.PermUsersManage)).Post("/api/v1/users", authH.CreateUser)
		r.With(rbac.RequirePermission(models.PermUsersManage)).Put("/api/v1/users/{id}", resourceH.Update("users"))
		r.With(rbac.RequirePermission(models.PermUsersManage)).Delete("/api/v1/users/{id}", authH.DeleteUser)
		r.With(rbac.RequirePermission(models.PermUsersManage)).Put("/api/v1/users/{id}/role", resourceH.Update("users"))
		...
		r.With(rbac.RequirePermission(models.PermSettingsWrite)).Get("/api/v1/isp-links", resourceH.List("isp_links"))
		r.With(rbac.RequirePermission(models.PermSettingsWrite)).Get("/api/v1/isp-links/comparison", ispH.Comparison)
		r.With(rbac.RequirePermission(models.PermSettingsWrite)).Get("/api/v1/isp-links/{id}", resourceH.Get("isp_links"))
		...
		r.With(rbac.RequirePermission(models.PermSettingsWrite)).Post("/api/v1/isp-links", resourceH.Create("isp_links"))
		r.With(rbac.RequirePermission(models.PermSettingsWrite)).Put("/api/v1/isp-links/{id}", resourceH.Update("isp_links"))
		r.With(rbac.RequirePermission(models.PermSettingsWrite)).Delete("/api/v1/isp-links/{id}", resourceH.Delete("isp_links"))

		// --- SLA (sla.manage) ---
		r.With(rbac.RequirePermission(models.PermSLAManage)).Get("/api/v1/sla", resourceH.List("sla_definitions"))
		r.With(rbac.RequirePermission(models.PermSLAManage)).Put("/api/v1/sla/{id}", resourceH.Update("sla_definitions"))
