...
interface Props { value: number; size?: number; strokeWidth?: number; label?: string; className?: string; showParticles?: boolean; }
export default function RingGauge({ value, size = 128, strokeWidth = 9, label, className, showParticles = false }: Props) {
  ...
  return <div className={`relative inline-grid place-items-center ${className ?? ''}`} style={{ width: size, height: size }}>
    ...
    <span className="absolute font-headline text-3xl font-semibold tabular-nums" style={{ color }}><AnimatedCounter value={score} decimals={score % 1 ? 1 : 0} /></span>
