  ...
  return (
    <div className="space-y-6 report-section">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Severity donut */}
        <div className="bg-surface-container-low rounded-lg p-6 border border-outline-variant/20 flex flex-col items-center">
          <h4 className="text-xs font-semibold uppercase tracking-wide text-on-surface-variant mb-4">Severity Breakdown</h4>
          {alerts.length === 0 ? (
        ...
        <div className="lg:col-span-2 bg-surface-container-low rounded-lg p-6 border border-outline-variant/20">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h4 className="text-xs font-semibold uppercase tracking-wide text-on-surface-variant">Alert Timeline</h4>
          ...
          {alerts.length === 0 ? (
            ...
            <div className="overflow-y-auto max-h-[420px] space-y-2 pr-1">
              {alerts.map((a) => {
                ...
                return (
                  <div key={a.id} className={`flex items-start gap-3 p-3 rounded-lg border ${bg} transition-[filter] hover:bg-surface-container-low`}>
                    <span className="material-symbols-outlined mt-0.5 text-sm" style={{ color }}>{SEV_ICONS[a.severity] || 'info'}</span>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-0.5">
                        <span className="font-headline font-semibold text-xs text-on-surface truncate">{a.deviceName || `Device ${a.deviceId}`}</span>
                        <span className="text-[10px] font-data text-on-surface-variant shrink-0">{new Date(a.createdAt).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                      </div>
                      <p className="text-xs text-on-surface-variant truncate">{a.message}</p>
                    </div>
                    <span className={`shrink-0 px-2 py-0.5 rounded-full text-[9px] font-semibold uppercase border`} style={{ color, borderColor: `${color}40` }}>{a.severity}</span>
                  </div>
                );
              })}
            </div>
