    ...
    <div className="flex flex-col items-center">
      <svg width={160} height={160} className="transform -rotate-90">
        ...
        <circle cx={cx} cy={cy} r={r} fill="none" stroke={color} strokeWidth={10}
          ...
          style={{ '--gauge-circumference': circ, '--gauge-offset': circ - (pct / 100) * circ, filter: `drop-shadow(0 0 8px ${color}40)` } as React.CSSProperties} />
      </svg>
      <div className="absolute flex flex-col items-center" style={{ marginTop: 48 }}>
        <span className="font-headline text-3xl font-black" style={{ color }}>{pct.toFixed(2)}%</span>
        <span className="text-[10px] text-on-surface-variant uppercase tracking-widest mt-1">Availability</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6 report-section">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-surface-container-high rounded-xl p-6 border border-outline-variant/20 flex flex-col items-center justify-center relative">
          <p className="text-[10px] text-on-surface-variant uppercase tracking-[0.2em] mb-4">SLA Target: {SLA_TARGET}%</p>
          <SlaGauge value={avail} />
          <div className={`mt-6 inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest ${met ? 'bg-primary/10 text-primary border border-primary/30' : 'bg-error/10 text-error border border-error/30'}`}>
        ...
        <div className="lg:col-span-2 bg-surface-container-high rounded-xl p-6 border border-outline-variant/20">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div><p className="text-[10px] text-on-surface-variant uppercase tracking-widest">Target</p><p className="font-headline text-xl font-bold text-on-surface">{SLA_TARGET}%</p></div>
            <div><p className="text-[10px] text-on-surface-variant uppercase tracking-widest">Actual</p><p className={`font-headline text-xl font-bold ${avail >= SLA_TARGET ? 'text-primary' : 'text-error'}`}>{avail}%</p></div>
            <div><p className="text-[10px] text-on-surface-variant uppercase tracking-widest">Buckets Met</p><p className="font-headline text-xl font-bold text-on-surface">{bucketsMet}/{totalBuckets}</p></div>
            <div><p className="text-[10px] text-on-surface-variant uppercase tracking-widest">Down Events</p><p className="font-headline text-xl font-bold text-error">{summary?.downSamples ?? 0}</p></div>
          </div>
          <h4 className="text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-2">Availability vs SLA Target</h4>
