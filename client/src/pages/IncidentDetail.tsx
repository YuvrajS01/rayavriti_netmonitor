    ...
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="flex items-start gap-4">
          <button onClick={() => navigate('/incidents')} className="mt-1 text-on-surface-variant hover:text-primary transition-colors">
            <span className="material-symbols-outlined">arrow_back</span>
          </button>
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="font-headline text-2xl font-bold text-on-surface">{incident.title}</h1>
              <span className={`text-xs font-bold uppercase px-2.5 py-0.5 rounded-full ${severityBgColor(incident.severity)} ${severityTextColor(incident.severity)}`}>{incident.severity}</span>
              <span className={`text-xs font-bold uppercase px-2.5 py-0.5 rounded-full ${statusBgColor(incident.status)} ${statusTextColor(incident.status)}`}>{incident.status}</span>
              {incident.sla_breached && <span className="text-xs font-bold uppercase px-2.5 py-0.5 rounded-full bg-error/10 text-error">SLA Breach</span>}
            </div>
          ...
          {STATUS_FLOW.map((s, i) => (
            <div key={s} className="flex-1 flex items-center">
              <div className="flex flex-col items-center flex-1">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${i <= statusIdx ? 'bg-primary text-on-primary' : 'bg-surface-container-highest text-outline'}`}>
                  {i < statusIdx ? <span className="material-symbols-outlined text-sm">check</span> : i + 1}
                </div>
                <span className={`text-[10px] mt-1 uppercase tracking-wide font-bold ${i <= statusIdx ? 'text-primary' : 'text-outline'}`}>{s}</span>
              </div>
              {i < STATUS_FLOW.length - 1 && <div className={`h-0.5 flex-1 mx-1 rounded ${i < statusIdx ? 'bg-primary' : 'bg-surface-container-highest'}`} />}
            </div>
          ))}
        </div>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {incident.description && (
            <Card variant="low" className="p-5">
              <h3 className="font-headline font-bold text-sm uppercase tracking-wide text-on-surface-variant mb-2">Description</h3>
          ...
          {incident.impact_description && (
            <Card variant="low" className="p-5">
              <h3 className="font-headline font-bold text-sm uppercase tracking-wide text-on-surface-variant mb-2">Impact</h3>
          ...
          {incident.resolution && (
            <Card variant="low" className="p-5">
              <h3 className="font-headline font-bold text-sm uppercase tracking-wide text-on-surface-variant mb-2">Resolution</h3>
          ...
          {incident.root_cause && (
            <Card variant="low" className="p-5">
              <h3 className="font-headline font-bold text-sm uppercase tracking-wide text-on-surface-variant mb-2">Root Cause</h3>
          ...
          <Card variant="low" className="overflow-hidden">
            <div className="p-5 border-b border-outline-variant/20">
              <h3 className="font-headline font-bold text-sm uppercase tracking-wide text-on-surface-variant">Timeline</h3>
            </div>
            {timeline.length === 0 ? (
                ...
                <div className="divide-y divide-outline-variant/10">
                  {timeline.map((entry) => (
                    <div key={entry.id} className="flex gap-4 p-5 relative">
                      <div className="w-[14px] h-[14px] rounded-full bg-surface-container-high border-2 border-outline-variant/50 flex-shrink-0 mt-1 z-10" style={{ marginLeft: '22px' }} />
        ...
        <div className="space-y-6">
          <Card variant="low" className="p-5 space-y-4">
            <h3 className="font-headline font-bold text-sm uppercase tracking-wide text-on-surface-variant">Details</h3>
            {[
          ...
          {devices.length > 0 && (
            <Card variant="low" className="overflow-hidden">
              <div className="p-5 border-b border-outline-variant/20">
                <h3 className="font-headline font-bold text-sm uppercase tracking-wide text-on-surface-variant">Affected Devices</h3>
              </div>
              <div className="divide-y divide-outline-variant/10">
                {devices.map((d) => (
                  <div key={d.id} className="px-5 py-3 flex items-center justify-between hover:bg-surface-container-high/50 transition-colors">
                    <div>
                      <div className="text-sm font-medium">{d.name}</div>
                      <div className="text-xs text-on-surface-variant font-mono">{d.ip}</div>
                    </div>
                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${statusBgColor(d.status)}/10 ${statusTextColor(d.status)}`}>{d.status}</span>
                  </div>
                ))}
              </div>
            </Card>
      ...
      {showResolve && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60" onClick={() => setShowResolve(false)}>
          <div className="bg-surface-container-low border border-outline-variant/30 rounded-lg w-full max-w-md max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-outline-variant/20 shrink-0"><h2 className="font-headline text-lg font-bold">Resolve Incident</h2></div>
            <div className="p-6 space-y-4 flex-1 min-h-0 overflow-y-auto">
              <div>
                <label className="text-[10px] text-on-surface-variant uppercase tracking-wide block mb-1">Resolution</label>
                <textarea value={resolveForm.resolution} onChange={(e) => setResolveForm({ ...resolveForm, resolution: e.target.value })} rows={3} className="w-full bg-surface-container-highest border border-outline-variant/20 rounded-lg px-4 py-2.5 text-sm text-on-surface placeholder:text-outline focus:ring-1 focus:ring-primary outline-none resize-none" placeholder="How was it resolved?" />
              </div>
              <div>
                <label className="text-[10px] text-on-surface-variant uppercase tracking-wide block mb-1">Root Cause</label>
                <input value={resolveForm.rootCause} onChange={(e) => setResolveForm({ ...resolveForm, rootCause: e.target.value })} className="w-full bg-surface-container-highest border border-outline-variant/20 rounded-lg px-4 py-2.5 text-sm text-on-surface placeholder:text-outline focus:ring-1 focus:ring-primary outline-none" placeholder="What caused it?" />
              </div>
              <div>
                <label className="text-[10px] text-on-surface-variant uppercase tracking-wide block mb-1">Category</label>
                <select value={resolveForm.rootCauseCategory} onChange={(e) => setResolveForm({ ...resolveForm, rootCauseCategory: e.target.value })} className="w-full bg-surface-container-highest border border-outline-variant/20 rounded-lg px-4 py-2.5 text-sm text-on-surface outline-none focus:ring-1 focus:ring-primary">
            ...
            <div className="flex border-t border-outline-variant/20 shrink-0">
              <button onClick={() => setShowResolve(false)} className="flex-1 py-3 text-xs font-headline font-bold uppercase tracking-wide text-on-surface-variant hover:bg-surface-container-high transition-colors">Cancel</button>
              <div className="w-px bg-outline-variant/20" />
              <button onClick={() => handleAction('resolve', resolveForm)} disabled={submitting || !resolveForm.resolution} className="flex-1 py-3 text-xs font-headline font-bold uppercase tracking-wide text-primary hover:bg-primary/10 transition-colors disabled:opacity-50">{submitting ? 'Resolving...' : 'Resolve'}</button>
            </div>
          </div>
        </div>
      )}
