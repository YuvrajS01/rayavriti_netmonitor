...
export default function Alerts() {
  ...
  // Group alerts by severity for the active tab
  ...
  return (
    <div className={`group bg-surface-container-low rounded-xl border-l-[6px] ${sc.border} p-5 flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all hover:bg-surface-container-high`}>
      ...
      <div className="flex items-center gap-4">
        {alert.status === 'active' && (
          <button onClick={() => onAck(alert.id)} className={`px-6 py-2 border-2 ${sc.border} ${sc.text} font-headline font-bold text-xs uppercase hover:bg-error hover:text-on-error transition-all active:scale-95`}>
            ACKNOWLEDGE
          </button>
        )}
